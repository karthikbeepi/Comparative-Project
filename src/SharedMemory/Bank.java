...
public class Bank implements Runnable {
	...
	public Bank(String s, Integer integer, Set<String> set, ArrayList<String> bankMailBox) {
		bankName = s;
		balance = integer;
		customers = new ArrayList<>();
		for(String temp : set)
			customers.add(temp);
		this.bankMailBox = bankMailBox;
	}
...
//		}
		...
		while (customersDone.size()!=customers.size()) {
			synchronized (bankMailBox) {
				while(bankMailBox.size()>0)
				{
					String req = bankMailBox.get(0);
					String rep = replyGenerator(req);
